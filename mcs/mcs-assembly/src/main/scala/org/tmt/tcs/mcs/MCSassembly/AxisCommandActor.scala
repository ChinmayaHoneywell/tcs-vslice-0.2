...
object AxisCommandActor {
  def createObject(commandResponseManager: CommandResponseManager,
    ...
    Behaviors.mutable(ctx => AxisCommandActor(ctx, commandResponseManager, hcdLocation, loggerFactory))
}

case class AxisCommandActor(ctx: ActorContext[ControlCommand],
  ...
  override def onMessage(controlCommand: ControlCommand): Behavior[ControlCommand] = {
    log.info(msg = s"Executing Axis command with id :  ${controlCommand.runId}")
    this
  }
}

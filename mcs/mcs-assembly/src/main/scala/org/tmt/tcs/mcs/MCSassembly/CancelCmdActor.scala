...
object CancelCmdActor {
  def createObject(commandResponseManager: CommandResponseManager,
    ...
    Behaviors.mutable(ctx => CancelCmdActor(ctx, commandResponseManager, hcdLocation, loggerFactory))
}

case class CancelCmdActor(ctx: ActorContext[ControlCommand],
  ...
  override def onMessage(msg: ControlCommand): Behavior[ControlCommand] = {
    log.info(msg = "Processing cancel command")
    this
  }
}

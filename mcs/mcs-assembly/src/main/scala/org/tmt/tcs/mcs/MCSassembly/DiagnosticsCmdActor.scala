...
object DiagnosticsCmdActor {
  def createObject(commandResponseManager: CommandResponseManager,
    ...
    Behaviors.mutable(ctx => DiagnosticsCmdActor(ctx, commandResponseManager, hcdLocation, loggerFactory))
}
case class DiagnosticsCmdActor(ctx: ActorContext[ControlCommand],
  ...
  override def onMessage(controlCommand: ControlCommand): Behavior[ControlCommand] = {
    log.info(msg = s"Executing Diagnostics command ${controlCommand}")
    this
  }
}
